...
namespace KumoShopMVC.Services
{
	public class VnPayService : IVnPayService
	{
		...
		public VnPaymentResponseModel PaymentExecute(IQueryCollection collections)
		{
			...
			{
				Console.WriteLine($"Transaction Reference ID: {vnp_orderId}");
			}
			else
			{
				Console.WriteLine("Invalid or missing vnp_TxnRef.");
				vnp_orderId = -1; 
			}

			if (long.TryParse(vnp_TransactionNo, out var vnp_TransactionId))
			{
				Console.WriteLine($"Transaction Number: {vnp_TransactionId}");
			}
			else
			{
				Console.WriteLine("Invalid or missing vnp_TransactionNo.");
				vnp_TransactionId = -1; 
			}
